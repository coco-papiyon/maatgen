...
Web版のGitHub監視設定では、監視ルールの作成・編集フォームをルール一覧上のポップアップとして表示する。Issue／PRには担当者loginを条件指定でき、PRには加えてレビュー依頼中ユーザーのloginをレビューア条件として指定できる。（TODO: コンフリクト条件はまだ実装されていない）レビュー依頼の変更はGitHub itemの正規化状態と変更検知hashに含める。ポーラーはOpenのIssue／PRだけを取得し、リポジトリの全クローズ履歴を毎回走査しない。VS Code版は監視ルール編集画面を持たず、自動実行されたSessionの参照のみ提供する。
