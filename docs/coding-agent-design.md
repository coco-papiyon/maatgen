...
Web版のGitHub監視設定では、監視ルールの作成・編集フォームをルール一覧上のポップアップとして表示する。Issue／PRには担当者loginを条件指定でき、Issue／PRの状態はOpen、Close、Open+Closeから選択できる（未指定はOpen）。PRには加えてレビュー依頼中ユーザーのloginをレビューア条件として指定できる。レビュー依頼の変更はGitHub itemの正規化状態と変更検知hashに含める。VS Code版は監視ルール編集画面を持たず、自動実行されたSessionの参照のみ提供する。
