...
サーバ設定画面では上位サーバごとの設定を横一列に表示する。再試行間隔（既定5分）と失敗上限（既定3回）は全上位サーバ共通の`upstreamRetry`設定として保存し、`GET/PUT /api/v1/upstream-retry-settings`で変更する。最初の接続は即時に試み、失敗後は設定した間隔を空けて再試行する。接続失敗が連続して上限に達すると、その接続は`stopped`となり、同じManagerプロセスでは自動再試行せず、サーバ選択肢からも外す。接続成功時に失敗回数はリセットする。設定は有効のまま保存し、Manager再起動時には再び接続を試みる。設定画面の「再接続」は`POST /api/v1/upstreams/{id}/reconnect`で停止した接続の試行を再開する。

右上のサーバ選択プルダウンは選択専用とし、「ノードを追加」項目や追加ダイアログは表示しない。
