...
`yamux.Session`は双方向にストリームを開けるため、上位ノードも同じSessionに通常のHTTPハンドラを`http.Serve`する。下位ノードのWeb UIがNode Nameで表示される`upstream`を選択した場合は、`/api/upstream/...`をこの逆方向ストリーム経由で上位ノードへ転送する。`Local`は常にブラウザが直接開いている下位ノードを指す。双方向化に新しいTCP接続やブラウザ向け公開ポートは必要ない。
...
`POST /api/nodes`による事前登録機能は維持するが、右上のサーバ選択プルダウンは選択専用とし、「ノードを追加」操作や追加ダイアログは配置しない。

1. APIクライアントが表示名（例：`Linux dev box`）を指定して登録する。
...
事前登録は必須ではない。Decision 1の自動登録動作（下位ノードが起動フラグで任意の`node-id`／`node-name`を指定し、事前登録なしにそのまま接続する）は変更しない。上位側で先に追加していない`nodeId`が接続してきた場合も、`pending`を経由せず直接`connected`として登録する。

### 3.2 下位ノードは複数の上位ノードへ同時に接続できる（Scope and non-goalsの当初の除外を撤回）

当初、下位ノードが複数の上位ノードへ同時接続する構成は対象外としていたが、実際の運用で複数の上位ノードを使い分けたいという要求が生じたため、この制限を撤回する。1台の下位ノードから複数の上位ノードへ、それぞれ独立した接続・再接続ループで同時に接続できる。

- `UpstreamConfig`（`internal/protocol/relay.go`）は`id`フィールドを持ち、下位ノードの「サーバ」設定画面は0件以上の`UpstreamConfig`の一覧を保持・永続化する（`toolconfig.Config.Upstreams []protocol.UpstreamConfig`、`config/providers.json`の`"upstreams"`配列）。
- `internal/relay.ClientSupervisor`は、Decision 1で述べた「同時に1本だけ」というシングルトン前提を撤回し、`id`ごとに独立した接続試行ループ（バックオフ・再接続を含む）を保持する。ある`id`の追加・変更・削除は他の`id`の接続に影響しない。
- APIは`GET/POST /api/v1/upstreams`（一覧・新規追加）と`PUT/DELETE /api/v1/upstreams/{id}`（更新・削除）のCRUDとし、単一設定用の`GET/PUT /api/v1/upstream`は廃止した。無効化は`enabled: false`のPUTで表現する（Decision 4の下位ノードレジストリと異なり、こちらは自分自身の発信接続なので接続中でも削除してよい）。
- 上位ノードへのリバースプロキシも`/api/upstream/`単一パスから`/api/upstreams/{id}/`へ変更し、Web UIのノードセレクタは設定済みの上位ノードそれぞれを独立した仮想ノード（`upstream:{id}`）として列挙する（Decision 5の「単一ノード切替方式」自体は変更しない。切替対象の選択肢が複数の上位ノードに増えるだけである）。
- `--upstream-url`等の起動フラグは、保存済みの一覧に追加でもう1本、このプロセスの実行中だけの接続を張るブートストラップとして扱う（固定id、`toolConfig.Upstreams`には永続化しない）。
...
`GET /api/nodes`は、ローカル自身（`local`固定ID）と、上記3状態いずれかの下位ノード一覧（id／name／status／最終接続時刻）を返す。`POST /api/nodes`はAPI経由の事前登録用に`pending`エントリを新規作成し、`nodeId`／`nodeToken`／起動コマンド文字列を返す。`DELETE /api/nodes/{nodeId}`は`pending`または`disconnected`のエントリの履歴を消す（`connected`に対しては`409 node_connected`を返し削除しない）。
...
- サーバセレクタは既存ノードの選択だけを提供し、追加操作は表示しない。
...
対象は、下位ノードから上位ノードへのアウトバウンド接続確立、yamuxによるストリーム多重化、既存HTTP/WebSocket APIのプロキシ転送、ノードレジストリ、Web UIの選択専用ノードセレクタ、`POST /api/nodes`によるAPI経由のpendingノード生成とする。
...
6. 右上のサーバ選択プルダウンは選択専用とし、ノード追加ダイアログを持たせない。
