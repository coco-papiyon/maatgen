...
| D-68 | 上位サーバの接続失敗 | 共通設定の再試行間隔（既定5分）と失敗上限（既定3回）で自動再試行を停止し、手動再接続またはManager再起動で再開する | `upstreamRetry`を設定ファイルへ保存し、`GET/PUT /api/v1/upstream-retry-settings`から変更する。`UpstreamStatus`へ`stopped`と失敗回数を追加し、停止中は上位サーバ選択肢に含めない。設定画面の`POST /api/v1/upstreams/{id}/reconnect`で試行を再開する | サーバ設定・Relay | 検討済み |
...
- [x] ポーラーはOpenのIssue／PRだけを取得し、状態条件は設定画面に公開しない
