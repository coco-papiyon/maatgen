...
Windowsでビルドから起動までを一度に行う場合は、リポジトリ直下の`build-and-start.bat`を実行してください。ビルドが成功するとAgent Managerが`http://127.0.0.1:3100/`で起動します。終了するには、起動したウィンドウで`Ctrl+C`を押してください。
