ALTER TABLE sessions RENAME TO sessions_codex_only;
...
FROM sessions_codex_only;

DROP TABLE sessions_codex_only;
CREATE INDEX sessions_created_at_idx ON sessions(created_at DESC);

ALTER TABLE events RENAME TO events_codex_only;
...
CREATE TABLE events (
    ...
    source TEXT NOT NULL CHECK (source IN ('user', 'codex', 'copilot', 'manager')),
...
SELECT id, session_id, run_id, sequence, source, event_type, 2, payload_json, created_at
FROM events_codex_only;

DROP TABLE events_codex_only;
